package lesson11.hashtable.project.one;
...
public class Employee {
	...
	public void printPaymentAmount(String date) {
		...
		if(salaryRecord.containsKey(date)) {
			...
			System.out.println(firstName + lastName + "did not receive a paycheck on " 
					+date);
		}
	...
	public void printAveragePaycheck() {
		...
		double sum = 0;
		for(String k : salaryRecord.keySet()) {
				sum += salaryRecord.get(k);
				System.out.println(salaryRecord.get(k));
		}
		
		System.out.println("Average paycheck for " +firstName + lastName + 
							" was " + sum/salaryRecord.size());
